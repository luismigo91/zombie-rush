...
- [x] 5.7 Commit "art-pass: migrar a URP 2D + post-proceso"

## 6. Personajes — spritesheets (fase 5)

- [x] 6.1 Crear `Assets/Resources/Art/characters/`; descargar spritesheets CC0/CC4-BY de soldado, zombie y boss (Kenney/itch); actualizar `ATTRIBUTION.md`
- [x] 6.2 Rebanar sheets (`Sprite Mode=Multiple`) o documentar sub-rects para `Sprite.Create` por código si vienen como textura única
- [x] 6.3 Extender `ArtCache` para servir arrays de `Sprite` por animación (`soldier_march`, `soldier_shoot`, `zombie_shamble`, `zombie_die`, `boss_idle`…)
- [x] 6.4 Elegir estrategia de tinte por tipo de zombie: sheet en grises + `SpriteRenderer.color`, o 3 paletas/sheets; documentar la decisión
- [x] 6.5 Actualizar `Squad` para que use `ArtCache` en vez de `PixelArt.SoldierMarch`/`SoldierShoot`; conservar `SpriteAnim`
